...
@FeignClient(value = "tasca", url = "${es.caib.comanda.client.base.url}/tasques")
public interface TascaServiceClient {

    @GetMapping(value = "/{id}")
    EntityModel<Tasca> getOne(
            @PathVariable("id") final Long id,
            @RequestParam(value = "perspectives", required = false) final String[] perspectives,
            @RequestHeader("Authorization") final String authorizationHeader);

    @GetMapping
    PagedModel<EntityModel<Tasca>> find(
