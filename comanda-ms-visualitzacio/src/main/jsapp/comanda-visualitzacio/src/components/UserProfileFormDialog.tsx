...
const UserProfileForm = () => {
    ...
    return <Grid container spacing={1} sx={{ px: 1, }}>
        ...
        {!data[UsuariModel.EMAIL] && !data[UsuariModel.EMAIL_ALTERNATIU] && (
            <Grid size={{ xs: 12 }}>
                <Alert severity="info">{t($ => $.menu.user.options.profile.form.noEmail, { autogeneratedEmail: data[UsuariModel.AUTOGENERATED_EMAIL] })}</Alert>
            </Grid>
        )}
        <Grid size={{ xs: 12 }}>
            <Divider sx={{ my: 2 }}>
    ...
    return (
        <MuiFormDialog
            ...
            formComponentProps={{
                ...
                perspectives: ["PREVIEW_AUTOGENERATED_EMAIL"],
            }}
        >
            <UserProfileForm/>
        </MuiFormDialog>
