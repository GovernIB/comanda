...
public class AppInfoEntornAppHelper {
    ...
    private void logicaCanviVersioRevisio(EntornAppEntity entornAppEntity, String versioAntiga, String revisioAntiga, String versioNova, String revisioNova) {
        ...
        if (haCanviatVersio || haCanviatRevisio) {
            ...
            // No passa per afterUpdateSave (aquest procés muta l'entitat directament, tant des del
            // botó manual de refresc com des del programador cada 15 min), cal notificar-ho
            // explícitament perquè el llistat i el detall de Salut es refresquin via SSE.
            eventPublisher.publishEvent(new ComandaSsePublishRequest(
                new ComandaSseEvent(ComandaSseEventTypes.ENTORN_APP_CHANGED, entornAppEntity.getId(), LocalDateTime.now())));
        }
    }
