...
CREATE
OR REPLACE VIEW SYS.comanda_v_sessions AS
...
CREATE
OR REPLACE VIEW SYS.comanda_v_segment_stats AS
...
CREATE
OR REPLACE VIEW SYS.comanda_v_top_sql AS
SELECT sql_id,
       elapsed_time,
       executions,
       buffer_gets,
...
CREATE
OR REPLACE VIEW SYS.comanda_v_sysstat AS
SELECT name, value
FROM v$sysstat
...
CREATE
OR REPLACE VIEW SYS.comanda_v_tablespaces AS
...
WHERE a.tablespace_name IN (SELECT DISTINCT tablespace_name
                            FROM dba_tables
                            WHERE owner = 'COMANDA'
...
CREATE
OR REPLACE VIEW SYS.comanda_v_bloquejos AS
SELECT s.sid,
...
GRANT SELECT ON SYS.comanda_v_bloquejos TO WWW_COMANDA;
-- comanda;
...
CREATE
OR REPLACE PUBLIC SYNONYM comanda_v_sessions      FOR SYS.comanda_v_sessions;
CREATE
OR REPLACE PUBLIC SYNONYM comanda_v_segment_stats FOR SYS.comanda_v_segment_stats;
CREATE
OR REPLACE PUBLIC SYNONYM comanda_v_top_sql       FOR SYS.comanda_v_top_sql;
CREATE
OR REPLACE PUBLIC SYNONYM comanda_v_sysstat       FOR SYS.comanda_v_sysstat;
CREATE
OR REPLACE PUBLIC SYNONYM comanda_v_tablespaces   FOR SYS.comanda_v_tablespaces;
CREATE
OR REPLACE PUBLIC SYNONYM comanda_v_bloquejos     FOR SYS.comanda_v_bloquejos;
