...
class SalutInfoHelperParameterizedTest {
    ...
    void compactar_quanCanvienMinutsHoresIDies_creaIActualitzaElsAgregatsEsperats(List<Step> steps) {
        ...
        for (int i = 0; i < steps.size(); i++) {
            ...
            if (i == 0) {
                ...
                assertThat(agregats.getOrDefault(TipusRegistreSalut.MINUTS, List.of())).hasSize(1);
                assertThat(lastAgg.get(TipusRegistreSalut.HORA)).isNotNull();
                assertThat(lastAgg.get(TipusRegistreSalut.DIA)).isNotNull();
            }
            if (i == 3) {
                List<SalutEntity> mins = agregats.getOrDefault(TipusRegistreSalut.MINUTS, List.of());
                assertThat(mins).hasSize(1);
                SalutEntity agg = mins.get(0);
                assertThat(agg.getNumElements()).isEqualTo(4);
                assertThat(agg.getAppLatenciaMitjana()).isEqualTo((100 + 200 + 300 + 400) / 4);
                SalutIntegracioEntity api = agg.getSalutIntegracions().stream().filter(x -> "API".equals(x.getCodi())).findFirst().orElseThrow();
                assertThat(api.getTotalTempsMig()).isEqualTo(400);
                assertThat(api.getTotalOk()).isEqualTo(30L);
                assertThat(api.getTotalError()).isEqualTo(20L);
                assertThat(api.getTempsMigUltimPeriode()).isEqualTo(570);
                assertThat(api.getPeticionsOkUltimPeriode()).isEqualTo(7L);
                assertThat(api.getPeticionsErrorUltimPeriode()).isEqualTo(3L);
                assertThat(agg.getSalutMissatges())
                        .anyMatch(x -> "m1".equals(x.getMissatge()) && x.getNivell() == SalutNivell.WARN)
                        .anyMatch(x -> "m2".equals(x.getMissatge()));
                assertThat(agg.getSalutDetalls())
                        .anyMatch(x -> "d1".equals(x.getCodi()) && "v1b".equals(x.getValor()))
                        .anyMatch(x -> "d2".equals(x.getCodi()) && "y".equals(x.getValor()));
            }
            if (i == 4) {
                List<SalutEntity> mins = agregats.getOrDefault(TipusRegistreSalut.MINUTS, List.of());
                ...
                assertThat(prev.getNumElements()).isEqualTo(4);
                assertThat(curr.getNumElements()).isEqualTo(1);
            }
            if (i == 5) {
                List<SalutEntity> hores = agregats.getOrDefault(TipusRegistreSalut.HORA, List.of());
                assertThat(hores.size()).isGreaterThanOrEqualTo(2);
                assertThat(lastAgg.get(TipusRegistreSalut.HORA).getData()).isEqualTo(steps.get(5).time);
            }
            if (i == 6) {
                List<SalutEntity> dies = agregats.getOrDefault(TipusRegistreSalut.DIA, List.of());
                assertThat(dies.size()).isGreaterThanOrEqualTo(2);
                assertThat(lastAgg.get(TipusRegistreSalut.DIA).getData()).isEqualTo(steps.get(6).time);
            }
        }

        SalutEntity primerMinuts = agregats.get(TipusRegistreSalut.MINUTS).get(0);
        assertThat(primerMinuts.getAppPctUp()).isNotNull();
        assertThat(primerMinuts.getBdPctUp()).isNotNull();
    }
}
