 ...
 */
// S'ha canviat l'anotació de @Length per @Size
// S'han afegit els input types pels següents tipus de camp: enums, referències, byte[], boolean, Duration
...
public class PropertyUtils {
	...
	private static class Jsr303AwarePropertyMetadata extends DefaultPropertyMetadata {
		...
		public String getInputType() {
			...
			Class<?> resolvedType = property.getType().resolve();
			if (resolvedType != null) {
				if (boolean.class.isAssignableFrom(resolvedType) || Boolean.class.isAssignableFrom(resolvedType)) {
					return "checkbox";
				}
				if (Duration.class.isAssignableFrom(resolvedType)) {
					return "undefined";
				}
				if (ResourceReference.class.isAssignableFrom(resolvedType) || resolvedType.isEnum()) {
					return "search";
				}
				if (resolvedType.isArray() && byte.class.equals(resolvedType.getComponentType())) {
					return "file";
				}
			}
